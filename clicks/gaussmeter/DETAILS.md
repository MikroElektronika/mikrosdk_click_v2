...
> This example showcases how to configure and use the Gaussmeter click. This click measures
  magnetic fields around the device using a 3 axis measurement system. Alongside the magneto-
  meter, the click contains an integrated temperature sensor which provides data for the the-
  rmal compensation. 
...
> This function initializes and configures the click and logger modules. Additional configu-
  ring is done in the default_cfg(...) function. 
...
void application_task ( )
{
    ...
    for ( cnt = 0; cnt < 4; cnt++ )
    {
        switch ( gaussmeter.aux.command_byte_low & axis_check )
        {
            case 1:
            {
                log_printf( &logger, " * Temperature (16-bit): %.2f C\r\n", temp_buf[ index++ ] );
                break;
            }
            case 2:
            {
