...
> [GNSS MAX 4 Click](https://www.mikroe.com/?pid_product=MIKROE-6994) demo application is developed using
the [NECTO Studio](https://www.mikroe.com/necto), ensuring compatibility with [mikroSDK](https://www.mikroe.com/mikrosdk)'s
open-source libraries and tools. Designed for plug-and-play implementation and testing, the demo is fully compatible with
all development, starter, and mikromedia boards featuring a [mikroBUS&trade;](https://www.mikroe.com/mikrobus) socket.

<p align="center">
  <img src="https://www.mikroe.com/?pid_product=MIKROE-6994&image=1" height=300px>
</p>
...
- `gnssmax4_cfg_setup` This function initializes Click configuration structure to initial values.
```c
void gnssmax4_cfg_setup ( gnssmax4_cfg_t *cfg );
```

- `gnssmax4_init` This function initializes all necessary pins and peripherals used for this Click board.
```c
err_t gnssmax4_init ( gnssmax4_t *ctx, gnssmax4_cfg_t *cfg );
```

- `gnssmax4_get_pps_pin` This function returns the pulse per second (PPS) pin logic state.
```c
uint8_t gnssmax4_get_pps_pin ( gnssmax4_t *ctx );
```

- `gnssmax4_set_exi_pin` This function sets the EXI (EXTINT) pin logic state.
```c
void gnssmax4_set_exi_pin ( gnssmax4_t *ctx, uint8_t state );
```

- `gnssmax4_reset_device` This function resets the device by toggling the RST pin.
```c
void gnssmax4_reset_device ( gnssmax4_t *ctx );
```

- `gnssmax4_parse_gga` This function parses the GGA data from the read response buffer.
```c
