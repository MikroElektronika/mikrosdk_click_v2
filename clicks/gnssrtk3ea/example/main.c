 ...
 *
 * ## Additional Function
 * - static void gnssrtk3ea_clear_app_buf ( void )
 * - static void gnssrtk3ea_log_app_buf ( void )
 * - static err_t gnssrtk3ea_process ( gnssrtk3ea_t *ctx )
 * - static void gnssrtk3ea_parser_application ( uint8_t *rsp )
 *
...
static gnssrtk3ea_t gnssrtk3ea;
static log_t logger;
...
/**
 * @brief GNSS RTK 3 EA clearing application buffer.
...
/**
 * @brief GNSS RTK 3 EA log application buffer.
...
/**
 * @brief GNSS RTK 3 EA parser application.
...
static err_t gnssrtk3ea_process ( gnssrtk3ea_t *ctx ) 
{
    ...
    if ( ( GNSSRTK3EA_DRV_SEL_I2C == ctx->drv_sel ) && ( !i2c_data_ready ) )
    {
        uint16_t pps_wait_log_cnt = 0;
        while ( !gnssrtk3ea_get_pps_pin ( ctx ) )
        {
    ...
    rx_size = gnssrtk3ea_generic_read( ctx, rx_buf, PROCESS_BUFFER_SIZE );
...
static void gnssrtk3ea_parser_application ( uint8_t *rsp )
{
    ...
    if ( GNSSRTK3EA_OK == gnssrtk3ea_parse_gga( rsp, GNSSRTK3EA_GGA_LATITUDE, element_buf ) )
    {
        ...
        if ( strlen( element_buf ) > 0 )
        {
            ...
            gnssrtk3ea_parse_gga( rsp, GNSSRTK3EA_GGA_LONGITUDE, element_buf );
            log_printf( &logger, " Longitude: %.3s degrees, %s minutes \r\n", element_buf, &element_buf[ 3 ] );
            memset( element_buf, 0, sizeof( element_buf ) );
            gnssrtk3ea_parse_gga( rsp, GNSSRTK3EA_GGA_ALTITUDE, element_buf );
