...
> [Hall Current 25 Click](https://www.mikroe.com/?pid_product=MIKROE-6916) demo application is developed using
the [NECTO Studio](https://www.mikroe.com/necto), ensuring compatibility with [mikroSDK](https://www.mikroe.com/mikrosdk)'s
open-source libraries and tools. Designed for plug-and-play implementation and testing, the demo is fully compatible with
all development, starter, and mikromedia boards featuring a [mikroBUS&trade;](https://www.mikroe.com/mikrobus) socket.

<p align="center">
  <img src="https://www.mikroe.com/?pid_product=MIKROE-6916&image=1" height=300px>
</p>
...
- `hallcurrent25_cfg_setup` This function initializes Click configuration structure to initial values.
```c
void hallcurrent25_cfg_setup ( hallcurrent25_cfg_t *cfg );
```

- `hallcurrent25_init` This function initializes all necessary pins and peripherals used for this Click board.
```c
err_t hallcurrent25_init ( hallcurrent25_t *ctx, hallcurrent25_cfg_t *cfg );
```

- `hallcurrent25_calib_offset` This function calibrates the zero current offset value.
```c
err_t hallcurrent25_calib_offset ( hallcurrent25_t *ctx );
```

- `hallcurrent25_calib_resolution` This function calibrates the data resolution at a known load current.
```c
err_t hallcurrent25_calib_resolution ( hallcurrent25_t *ctx, float calib_current );
```

- `hallcurrent25_read_current` This function reads the input current level [A].
```c
...
void application_init ( void )
{
    ...
    hallcurrent25_cfg_t hallcurrent25_cfg;  /**< Click config object. */
    ...
    HALLCURRENT25_MAP_MIKROBUS( hallcurrent25_cfg, MIKROBUS_POSITION_HALLCURRENT25 );
    ...
    log_printf( &logger, " Keep the load current set at %.1f A during the calibration process.\r\n", 
                HALLCURRENT25_CALIBRATING_CURRENT );
    for ( uint8_t cnt = 5; cnt > 0; cnt-- )
    {
        log_printf( &logger, " %u\r\n", ( uint16_t ) cnt );
        Delay_ms ( 1000 );
    }
    if ( HALLCURRENT25_ERROR == hallcurrent25_calib_resolution ( &hallcurrent25, HALLCURRENT25_CALIBRATING_CURRENT ) )
    {
