...
> [RS485 Isolator 5 Click](https://www.mikroe.com/?pid_product=MIKROE-6887) demo application is developed using
the [NECTO Studio](https://www.mikroe.com/necto), ensuring compatibility with [mikroSDK](https://www.mikroe.com/mikrosdk)'s
open-source libraries and tools. Designed for plug-and-play implementation and testing, the demo is fully compatible with
all development, starter, and mikromedia boards featuring a [mikroBUS&trade;](https://www.mikroe.com/mikrobus) socket.

<p align="center">
  <img src="https://www.mikroe.com/?pid_product=MIKROE-6887&image=1" height=300px>
</p>
...
- MikroSDK.Board
- MikroSDK.Log
- Click.RS485Isolator5

### Example Key Functions

- `rs485isolator5_cfg_setup` This function initializes Click configuration structure to initial values.
```c
void rs485isolator5_cfg_setup ( rs485isolator5_cfg_t *cfg );
```

- `rs485isolator5_init` This function initializes all necessary pins and peripherals used for this Click board.
```c
err_t rs485isolator5_init ( rs485isolator5_t *ctx, rs485isolator5_cfg_t *cfg );
```

- `rs485isolator5_generic_write` This function writes a desired number of data bytes by using UART serial interface.
```c
err_t rs485isolator5_generic_write ( rs485isolator5_t *ctx, uint8_t *data_in, uint16_t len );
```

- `rs485isolator5_generic_read` This function reads a desired number of data bytes by using UART serial interface.
```c
err_t rs485isolator5_generic_read ( rs485isolator5_t *ctx, uint8_t *data_out, uint16_t len );
```

- `rs485isolator5_receiver_enable` This function enables the receiver output by setting the RE pin to low logic state.
```c
void rs485isolator5_receiver_enable ( rs485isolator5_t *ctx );
```

- `rs485isolator5_driver_enable` This function enables the driver input by setting the DE pin to high logic state.
```c
...
void application_init ( void )
{
    ...
    RS485ISOLATOR5_MAP_MIKROBUS( rs485isolator5_cfg, MIKROBUS_POSITION_RS485ISOLATOR5 );
